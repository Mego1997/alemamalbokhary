            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header pb-0">
                        <h4 class="card-title">فواتير النثريات</h4>
                        <a class="btn btn-outline-primary btn-md mt-1 " href="{{ asset('/dashboard/nathriaat/create') }}">
                            إضافة فاتوره
                        </a>
                    ...
                    <div class="card-content collapse show">
                        <div class="card-body card-dashboard">
                            ...
                            @endif
                                ...
                                <p class="card-text"></p>
                                <table class="table table-striped table-responsive table-bordered zero-configuration">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th>الاسم</th>
                                ...
                                <tbody>
                                    @foreach ($invoices as $key => $invoice)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{$invoice->name }}</td>
                                            <td>{{$invoice->phone == null ? 'لايوجد' : $invoice->phone  }}</td>
                                            <td>{{ $invoice->paid }}</td>
                                            <td>{{ $invoice->created_at }}</td>
                                            <td>{{ $invoice->note == null ? 'لايوجد' : $invoice->note }}</td>
                                            <td>
                                                <a href="{{ route('nathriaat.show', $invoice->id) }}"
...
{{--                                            <form action="{{ route('donationinvoicepurchases.destroy', $invoice->id) }}" method="POST">--}}
{{--                                                @csrf--}}
{{--                                                @method('delete')--}}
